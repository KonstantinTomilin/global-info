...
import {
  buildRegionAwareAssetRef,
  regionFromSurfaceMetadata,
} from "../classic/materialize-report-assets";
...
export async function buildOrionGoldenAssets(input: {
  ...
  const imageAssets: ReportAssetV1[] = input.ctx.searchSurfaces
    ...
    .map((row, idx) => {
      ...
      const region = regionFromSurfaceMetadata(rm);
      const provider = String(row.provider ?? rm.provider ?? row.source ?? "google")
        .toLowerCase()
        .includes("yandex")
        ? "yandex"
        : "google";
      return {
        assetRef: buildRegionAwareAssetRef({
          prefix: "media",
          region,
          provider,
          surface: "image",
